...
for current_dataset in ["ArMIS", "MD-Agreement", "ConvAbuse", "HS-Brexit"]:  # loop on datasets
    for current_split in ["train", "dev"]:  # loop on splits
        current_file = (
            "./" + current_dataset + "_dataset/" + current_dataset + "_" + current_split + ".json"
        )
        data = json.load(open(current_file, encoding="UTF-8"))
...
for current_dataset in ["ArMIS", "MD-Agreement", "ConvAbuse", "HS-Brexit"]:  # loop on datasets
    for current_split in ["train"]:  # loop on splits, here only train
        ...
        data = json.load(open(current_file, encoding="UTF-8"))  # load data
...
soft_pred, hard_pred = get_data(
    "./majority_baseline_practicephase/res/ArMIS_results.tsv"
